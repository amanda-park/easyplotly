...
def histogram(df, x = "Predictor", y = None, color = None, facet_col = None, facet_row = None,
              bins = 20, opacity = 1,
                marginal = None, template = "ggplot2"):
    ...
    """
    x_clean, df_clean = clean_varname(df, var = x)
...
def scatterplot(df, x = "Predictor", y = "Response", color = None, jitter = False, jitter_sd = .1,
...
def linechart(df, x = "Predictor", y = "Response", color = None, template = "ggplot2"):
...
def control_chart_ADTK(df):
    ...
    fig = go.Figure(
        ...
        layout_title_text = "Control Chart Example",
...
if __name__ == '__main__':    
    #df = px.data.iris()
    #s = scatterplot(df, x = "sepal_width", y = "sepal_length", color = "species", marg_x = "histogram", marg_y = "box", trendline = "ols", opacity = .2)
    #plot(s)
